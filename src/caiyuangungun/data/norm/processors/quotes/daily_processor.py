...
"""
Daily Processor（日线行情数据处理器）

专门处理日线行情数据的清洗和标准化，包括：
- 剔除空值记录
- 重复记录验证
- 字段重命名
- 输出标准化parquet文件
...
logger = logging.getLogger('daily_processor')
...
class DailyProcessor(BaseProcessor):
    ...
    def __init__(self, config: Dict[str, Any]):
        ...
        # 扩展清洗函数 - 添加日线行情数据特有的功能
        self.cleaning_functions.update(self._register_daily_cleaning_functions())
        

    def _register_daily_cleaning_functions(self):
        """注册日线行情数据专用清洗函数"""
        return {
    ...
    def _get_grouping_fields(self, df: pd.DataFrame) -> List[str]:
        """获取分组字段 - 行情数据使用 ts_code + trade_date"""
...
def main():
    ...
    processor = DailyProcessor(test_config)
