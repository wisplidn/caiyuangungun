...
"""
AdjFactor Processor（复权因子数据处理器）

专门处理复权因子数据的清洗和标准化，包括：
- 剔除空值记录
- 重复记录验证
- 字段重命名
- 输出标准化parquet文件
...
logger = logging.getLogger('adj_factor_processor')


class AdjFactorProcessor(BaseProcessor):
    """复权因子数据专用处理器"""

    def __init__(self, config: Dict[str, Any]):
        super().__init__(config)
        
        # 扩展清洗函数 - 添加复权因子数据特有的功能
        self.cleaning_functions.update(self._register_adj_factor_cleaning_functions())
        

    def _register_adj_factor_cleaning_functions(self):
        """注册复权因子数据专用清洗函数"""
        return {
...
def main():
    ...
    processor = AdjFactorProcessor(test_config)
